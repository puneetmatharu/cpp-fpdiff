 ...
 ************************************************************************************/
std::vector<std::string> load_gzip(const std::string& filename)
{
  ...
  // Decompress data and pipe to stream
  unsigned int n_byte_read;
  unsigned char buffer[CHUNKSIZE];
  std::stringstream input_stream;
  while ((n_byte_read = gzread(file, buffer, CHUNKSIZE)))
  {
    input_stream << std::string{buffer, buffer + n_byte_read};
  }
  gzclose(file);

  // Parse streamed data line-by-line
  std::string line;
